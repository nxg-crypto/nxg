...
#include "ui_znodexchangecontroldialog.h"
...
std::list<std::string> ZNodeXchangeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNodeXchangeControlDialog::listMints;

ZNodeXchangeControlDialog::ZNodeXchangeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNodeXchangeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNodeXchangeControlDialog::~ZNodeXchangeControlDialog()
{
    delete ui;
}

void ZNodeXchangeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNodeXchangeControlDialog::updateList()
{
...
void ZNodeXchangeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNodeXchangeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNodeXchange_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNodeXchangeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNodeXchangeControlDialog::GetSelectedMints()
{
...
void ZNodeXchangeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
