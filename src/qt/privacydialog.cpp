...
#include "znodexchangecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNXG ought to be enough for anybody." - Bill Gates, 2017
    ui->zNXGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNXGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNXG->setEnabled(false);
        ui->pushButtonMintzNXG->setToolTip(tr("zNXG is currently disabled due to maintenance."));

        ui->pushButtonSpendzNXG->setEnabled(false);
        ui->pushButtonSpendzNXG->setToolTip(tr("zNXG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNXGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNXG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNXG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNXG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNXG in ") +
...
void PrivacyDialog::on_pushButtonSpendzNXG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNXG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNXG
        sendzNXG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNXG
    sendzNXG();
}

void PrivacyDialog::on_pushButtonZNodeXchangeControl_clicked()
{
    ZNodeXchangeControlDialog* zNXGControl = new ZNodeXchangeControlDialog(this);
    zNXGControl->setModel(walletModel);
    zNXGControl->exec();
}

void PrivacyDialog::setZNodeXchangeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNXGSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNXG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid NodeXchange Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNXGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNXG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NODEXCHANGE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNXG</b>";
    ...
    // use mints from zNXG selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNodeXchangeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNodeXchangeControlDialog::GetSelectedMints();
    }
    ...
    // Clear znodexchange selector in case it was used
    ZNodeXchangeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNXG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " NodeXchange, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNXG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNXG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNXG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNXG "));
    ui->labelzNXGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNXGSyncStatus->setVisible(fShow);
}
